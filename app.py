...
class User(UserMixin, db.Model):
    ...
    role = db.Column(db.String(20), nullable=False)  # financeiro, diretor, admin
...
class FechamentoCaixa(db.Model):
    ...
    dinheiro_saida = db.Column(db.Float, default=0.0)        # item 4 - saída não encerramento
    dinheiro_encerramento = db.Column(db.Float, default=0.0) # item 5 - movimento encerramento
    faturado = db.Column(db.Float, default=0.0)              # item 6
    uso_credito = db.Column(db.Float, default=0.0)           # item 7
    deposito_bancario = db.Column(db.Float, default=0.0)     # item 8
    cartao = db.Column(db.Float, default=0.0)                # item 9
    cortesia = db.Column(db.Float, default=0.0)              # item 10
    vendas_online = db.Column(db.Float, default=0.0)         # vendas online (opcional)
    ...
    # aguardando_financeiro → aguardando_diretor → aguardando_cofre → concluido
    ...
    def saude_score(self):
        """Retorna 0-100 baseado nos checks do diretor"""
...
def financeiro_check(fc_id):
    ...
    db.session.commit()
    return jsonify({'success': True, 'message': 'Conferência do Financeiro salva!'})
...
def diretor_check(fc_id):
    ...
    db.session.commit()
    return jsonify({'success': True, 'message': 'Conferência do Diretor salva!'})
